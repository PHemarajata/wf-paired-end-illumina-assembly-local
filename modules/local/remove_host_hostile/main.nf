process REMOVE_HOST_HOSTILE {
    ...
    script:
    def minFilesize = params.min_filesize_fastq_hostile_removed ?: "25000k"
    def min_filesize_output_fastq = ( ['c','b','k'].contains(minFilesize[-1])) ? "${minFilesize}" : "25000k"
    """
    # Set up environment to fix permission issues
    export HOME=\${PWD}/tmp_home
    export HOSTILE_DATA_DIR=\${PWD}/hostile_data
    mkdir -p \${HOME}/.local/share/hostile
    mkdir -p \${HOSTILE_DATA_DIR}
    
    # Source bash functions with proper path
