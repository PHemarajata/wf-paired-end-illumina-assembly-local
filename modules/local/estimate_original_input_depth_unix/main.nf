process ESTIMATE_ORIGINAL_INPUT_DEPTH_UNIX {
    ...
    input:
    // FIX: The process now takes a single tuple containing both inputs,
    // which is the result of the explicit .join() in the subworkflow.
    tuple val(meta), path(bp), path(size)
    ...
    if [ "${size}" -eq "0" ]; then
        msg "ERROR: Estimated genome size is zero for !{meta.id}, cannot calculate depth." >&2
        initial_depth=0
    else
    ...
    # Calculate the fraction of reads to subsample, preventing division by zero
    ...
    if ! [[ ${fraction_of_reads_to_use} =~ ^[0-9.]+$ ]]; then
      msg "ERROR: unable to calculate fraction of reads to use for !{meta.id}: ${fraction_of_reads_to_use}" >&2
      exit 1
    fi
    msg "INFO: Fraction of reads to use for !{meta.id}: ${fraction_of_reads_to_use}"
